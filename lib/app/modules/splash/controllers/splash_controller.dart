...
class SplashController extends GetxController {
  ...
  bool isLoading = true;
  Auth0 auth0 = Auth0('dev-xaoieegxph0mwc8w.us.auth0.com', 'Jz6KIuNPDDiWnjbeZfoU7Ecojs7gkSwN');
  ...
  void socialLogin() async {
    final credentials = await auth0
        .webAuthentication(
        scheme: "ehoa://dev-xaoieegxph0mwc8w.us.auth0.com/android/org.netgains.ehoa/callback")
        .login(useEphemeralSession: false);
        print(credentials);
  ...
  _socialLoginApiCall(String email) async {
    ...
    if (res.isNotEmpty) {
      ...
      Map<String, dynamic> profileMap = await ApiService().showProfile(id: (res['data']['user_id'].toString()));
      if (profileMap.isNotEmpty) {
        ...
        debugPrint("<<< Period day ${MySharedPref.getPeriodDay()}, PeriodLen ${MySharedPref.getPeriodLen()} >>>");     
      }
      socialLoader(false);
      Get.offAllNamed(AppPages.BASE);
    }
